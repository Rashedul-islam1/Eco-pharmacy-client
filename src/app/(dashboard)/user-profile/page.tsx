...
const useImageUpload = (storageKey: any) => {
  ...
  const handleChangeImage = async (e: any) => {
    ...
    if (fileInput.files.length > 0) {
      ...
      const newImageUrl = imageData?.data?.url;
      localStorage.setItem(storageKey, newImageUrl);
      setImageUrl(newImageUrl);
    }
  };

  useEffect(() => {
    const storedImageUrl: any = localStorage.getItem(storageKey);
    setImageUrl(storedImageUrl);
  }, [storageKey]);

  return { imageUrl, handleChangeImage };
};
const Profile = () => {
  const user = useSelector((state: RootState) => selectCurrentUser(state));
  const { imageUrl: cover, handleChangeImage: handleChangeCover } =
    useImageUpload("cover");

  const { imageUrl: profile, handleChangeImage: handleChangeProfile } =
    useImageUpload("profile");

  return (
    <div className="rounded-sm  bg-white  max-w-6xl mx-auto ">
      <div className="relative rounded-t overflow-hidden">
        <Image
          src={cover ? cover : CoverOne}
          alt="profile cover"
          width={1200}
          height={100}
          className="lg:h-[300px] h-[120px] md:h-[180px] w-full  rounded-t overflow-hidden object-cover object-center"
        />
        <form className="absolute bottom-1 right-1 ">
          <label
            htmlFor="cover"
      ...
      <div className="px-4 pb-6 text-center relative pt-16 md:pt-20 lg:pt-32 ">
        <div className="absolute -top-14 md:-top-20 left-0 right-0   mx-auto bg-[#575757] w-[110px] h-[110px] md:w-[150px] md:h-[150px] lg:w-[200px] lg:h-[200px] rounded-full  flex items-center justify-center ">
          <div className="relative  w-[100px] h-[100px] md:w-[140px] md:h-[140px] lg:w-[190px] lg:h-[190px] overflow-hidden">
            <Image
              ...
              src={
                profile || user?.photo || "https://i.ibb.co/ZMtqp3x/userp.jpg"
              }
              alt="profile"
              width={1200}
              height={100}
            />
            <form className="absolute bottom-4 -right-2">
              <label
                htmlFor="profile"
        ...
        <div className="">
          ...
          <p className="font-medium mb-3">{user?.role ? user?.role : "User"}</p>
          ...
          <div className="mt-6.5">
            <h4 className="mb-3.5 font-medium text-black ">Follow me on</h4>
